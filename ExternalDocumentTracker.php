...
require_once $_SERVER['DOCUMENT_ROOT'] . '/include/DbConstant.php';
class ExternalDocumentTracker {
    ...
    public static function get_neodms_connection(){
		return ExternalDocumentTracker::get_connection(
			DbConstant::NEODMS_CONNECTION_SERVERNAME,
			DbConstant::NEODMS_CONNECTION_USERNAME,
			DbConstant::NEODMS_CONNECTION_PASSWORD,
			DbConstant::NEODMS_CONNECTION_DBNAME);
	}

	public static function get_document_neodms_anes($vn, $an){
		if(DbConstant::NEODMS_CONNECTION_USE != 'Y'){
			return 0;
		}
		if($an != null && trim($an) != ''){
