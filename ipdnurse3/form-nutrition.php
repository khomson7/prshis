    ...
    <form id="audit_form">
        ...
        <div class="container-fluid">
            ...
            <div class="row align-items-center mb-3">
                ...
                <div class="col">
                    <h5 class="mb-0"><b>แบบประเมิน Nutrition และ Malnutrition</b></h5>
                </div>
                <div class="col-auto">
                    <?php if ($ids): ?>
                    ...
                    <div class="d-flex gap-3 flex-wrap">
                        <div class="flex-fill">
                            ...
                            <b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;เริ่มตั้งแต่ 17.00 - 18.49 เรียกว่า Mild protein-energy
                                nutrition</b><br>
                            <b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;เริ่มตั้งแต่ 16.00 - 16.99 เรียกว่า Moderate protein-energy
                                nutrition</b><br>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-12 mb-1">
                            <span class="section-label"><i class="fas fa-info-circle"></i> ข้อมูลผู้ป่วย (ดึงจากระบบ —
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="bmi-result-box" id="bmi_result_display">
                                ...
                                if ($f_bmi) {
                                    $bmi_v = (float) $f_bmi;
                                    if ($bmi_v < 16)
                                        echo '<span style="color:#dc3545;">Severe (' . number_format($bmi_v, 2) . ')</span>';
                                    elseif ($bmi_v < 17)
                                        echo '<span style="color:#fd7e14;">Moderate (' . number_format($bmi_v, 2) . ')</span>';
                                    elseif ($bmi_v < 18.5)
                                        echo '<span style="color:#e0a000;">Mild (' . number_format($bmi_v, 2) . ')</span>';
                                    else
                                        echo '<span style="color:#28a745;">Normal (' . number_format($bmi_v, 2) . ')</span>';
                                }
                                ?>
                            </div>
                            <input type="hidden" id="check_bmi" name="check_bmi"
                                value="<?= htmlspecialchars($f_check_bmi) ?>">
                        </div>
                    </div>
                    ...
                    <div class="row mb-2">
                        <div class="col-12 mb-2">
                            <span class="section-label">
                                <i class="fas fa-weight"></i> น้ำหนักก่อนรับไว้ (kg) — คลิก <i
                                    class="fas fa-lock-open text-primary"></i> เพื่อปลดล็อกแก้ไข
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <div class="bw-row">
                                    ...
                                    <div class="d-flex align-items-center mb-1">
                                        <span class="font-weight-bold flex-fill" style="font-size:0.85rem;">น้ำหนัก
                                            <?= $plabel ?></span>
                                        <button type="button" class="unlock-btn" id="btn_<?= $fid ?>"
                                            onclick="toggleReadonly('<?= $fid ?>')" title="คลิกเพื่อแก้ไข">
                                            <i class="fas fa-lock" id="icon_<?= $fid ?>"></i>
                                        </button>
                                    </div>
                                    <!-- BW input (readonly by default, unlock to edit) -->
                                    <div class="d-flex gap-1 mb-1">
                                        <input type="number" class="nutrition-input" id="<?= $fid ?>" name="<?= $fid ?>"
                                            value="<?= ($bw_vals[$fid] !== '' && $bw_vals[$fid] !== null) ? number_format((float) $bw_vals[$fid], 2, '.', '') : '' ?>"
                                            step="0.01" min="0" max="999.99" placeholder="0.00" readonly
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="bw-row" style="background:#fffde7;">
                                ...
                                <div class="percen-box" id="max_percent_display"
                                    style="font-size:1rem; min-height:40px; display:flex; align-items:center; justify-content:center;">
                                    <?php
                    ...
                    <div class="row mb-3">
                        <div class="col-12">
                            <div class="evaluation-summary-box" id="evaluation_summary_display">
...
<script type="text/javascript">
    ...
    function updateMaxPercent() {
        ...
        updateNutritionEvaluation();
    }

    // ---- Calculate final evaluation message (BMI vs Weight Loss) ----
    function updateNutritionEvaluation() {
        var bmi = parseFloat($('#bmi').val()) || 0;
        var bmi_severity = 0; // 0: None/Mild, 1: Moderate, 2: Severe
        var bmi_text = "";
        ...
        var periods = [
            ...
            { id: 'bw_2_3week', label: '2-3 สัปดาห์', mod: [2.1, 3], sev: 3 },
            { id: 'bw_1month', label: '1 เดือน', mod: [4.1, 5], sev: 5 },
            { id: 'bw_3month', label: '3 เดือน', mod: [7.1, 8], sev: 8 },
            { id: 'bw_5month', label: '>5 เดือน', mod: [8.1, 10], sev: 10 }
        ];
        ...
        if (bw_now > 0) {
            periods.forEach(function (p) {
                ...
                if (prev > 0) {
                    ...
                    if (current_p_sev >= wl_severity && current_p_sev > 0) {
                        // If same severity, take the higher percentage
                        if (current_p_sev > wl_severity || pct > max_pct) {
        ...
        if (bmi_severity > 0 || wl_severity > 0) {
            if (bmi_severity == wl_severity) {
                final_message = wl_text + " และ " + bmi_text;
            } else if (bmi_severity > wl_severity) {
                final_message = bmi_text;
            } else {
                final_message = wl_text;
            }
        }
