...
function auditEvaluationMessage(int $sum_score): string {
    if ($sum_score <= 7)      return '[AUDIT] ' . $sum_score . ' — Low risk drinker';
    elseif ($sum_score <= 15) return '[AUDIT] ' . $sum_score . ' — Hazardous drinker';
    elseif ($sum_score <= 20) return '[AUDIT] ' . $sum_score . ' — Harmful use';
    else                      return '[AUDIT] ' . $sum_score . ' — Alcohol dependence';
}
...
try {
    ...
    // -- 1. prs_alcohol --
    $sql_check = "SELECT id FROM prs_alcohol WHERE an = :an LIMIT 1";
    $stmt_check = $conn->prepare($sql_check);
    $stmt_check->execute(['an' => $an]);
    $existing_id = $stmt_check->fetchColumn();

    if ($existing_id) {
        $alcohol_id = $existing_id;
    ...
    echo json_encode(['status' => 'success', 'id' => $alcohol_id]);
