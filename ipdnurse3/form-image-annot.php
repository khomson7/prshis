...
// ---- โหลด record ตาม ?id= หรือสร้างใหม่ ----
...
if ($ids) {
    ...
    if (!$rec) {
        $ids = null;   // ไม่พบ → treat as new
    } else {
        $note_saved  = $rec['note']       ?? '';
        $created_by  = $rec['created_by'] ?? '';
        // แก้ไขได้เฉพาะผู้บันทึก
...
var undoStack    = [];
var bgImageObj   = null; // reference ภาพ background (ไม่ serialize ใน JSON)
var _savingUndo  = false; // ป้องกัน recursive call
...
function selectImage(idx) {
    ...
    fabric.Image.fromURL(img.b64, function(fimg) {
        ...
        canvas = new fabric.Canvas('c', {
            width: w, height: h,
            isDrawingMode: _canEdit,   // view-only → ปิด drawing
            selection: false,
        ...
        bgImageObj = fimg;          // เก็บ reference ไว้ restore หลัง Undo
        canvas.add(fimg);
...
// saveUndo: เก็บเฉพาะ annotation (ไม่รวม bgImageObj ซึ่งเป็น Data URL ใหญ่มาก)
function saveUndo() {
    if (!canvas || _savingUndo) return;
    _savingUndo = true;

    // ถอด bg ออกชั่วคราว → JSON จะเล็กลงมาก (ไม่มี Data URL ภาพ)
    if (bgImageObj) canvas.remove(bgImageObj);

    var s = JSON.stringify(canvas.toJSON());

    // ใส่ bg กลับ (object:added จะถูก block โดย _savingUndo)
...
// undoCanvas: restore annotation state แล้วค่อยใส่ bg กลับ
function undoCanvas() {
    if (!canvas || undoStack.length <= 1) return;
    undoStack.pop();
    _savingUndo = true;
    canvas.loadFromJSON(undoStack[undoStack.length - 1], function() {
        // ใส่ bg กลับหลัง restore เสมอ
        if (bgImageObj) {
...
// clearCanvas: ลบเฉพาะ annotation ไม่แตะ bgImageObj
...
// ============================================================
// combineAllImages — รวมทุกภาพใน imageList เป็น PNG เดียว (client-side)
// Layout: 1 ภาพ = เต็มความกว้าง, 2+ ภาพ = 2 คอลัมน์
// callback(dataURL) หรือ callback(null) ถ้าไม่มีภาพ
// ============================================================
function combineAllImages(callback) {
    ...
    var MAX_W = 1400;
    var GAP   = 6;
    var COLS  = imageList.length === 1 ? 1 : 2;
    var col_w = Math.floor((MAX_W - (COLS - 1) * GAP) / COLS);

    // โหลดภาพทั้งหมดพร้อมกัน
    var elems   = new Array(imageList.length);
    var pending = imageList.length;

    imageList.forEach(function(img, i) {
        var src = (img.annotatedB64 && img.annotatedB64.length > 100)
                  ? img.annotatedB64   // ภาพที่มีการวาด
                  : img.b64;           // ภาพต้นฉบับ
        var el = new Image();
        el.onload = function() {
            elems[i] = el;
            if (--pending === 0) drawCombined();
        };
        el.onerror = function() {
            elems[i] = null;
            if (--pending === 0) drawCombined();
    ...
    function drawCombined() {
        ...
        var cells = elems.map(function(el) {
            ...
            return { el: el,
                     w: Math.round(el.naturalWidth  * scale),
                     h: Math.round(el.naturalHeight * scale) };
        }).filter(Boolean);

        if (cells.length === 0) { callback(null); return; }

        // จัดเป็น rows (COLS ภาพต่อ row)
        var rows = [], row_h = [];
        for (var i = 0; i < cells.length; i += COLS) {
            var row = cells.slice(i, i + COLS);
            rows.push(row);
            row_h.push(Math.max.apply(null, row.map(function(c){ return c.h; })));
        }

        var total_h  = row_h.reduce(function(s, h){ return s + h; }, 0)
                       + GAP * (rows.length - 1);
        var canvas_w = COLS === 1 ? cells[0].w
...
$('#image_annot_form').on('submit', function(e) {
    ...
    // บันทึก SVG + annotatedB64 ของภาพที่กำลังแสดงอยู่
    saveSvgToCurrent();

    Swal.fire({ title:'กำลังรวมภาพ...', allowOutsideClick:false,
                didOpen:function(){ Swal.showLoading(); } });

    // รวมภาพทั้งหมดเป็น PNG เดียวก่อน submit
    combineAllImages(function(combinedB64) {
        ...
        var postData = {
            ...
            combinedB64: combinedB64 || '',   // ← ภาพรวม (อาจเป็น '' ถ้ารวมไม่ได้)
        };

        Swal.fire({ title:'กำลังบันทึก...', allowOutsideClick:false,
                    didOpen:function(){ Swal.showLoading(); } });

        var isNew = !id;   // ยังไม่มี id = INSERT ใหม่
