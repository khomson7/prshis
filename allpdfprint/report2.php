...
$mpdf2 = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'A4']);
$mpdf2->AddPageByArray([
        'margin-left' => 12,
        'margin-right' => 12,
        'margin-top' => 12,
        'margin-bottom' => 25,]);
...
';
        ...
        while ($row = $stmt->fetch()){
                ...
                $mpdf2->setFooter(' (พิมพ์โดย '.$_SESSION['name'].' วันที่พิมพ์ '.date('d/m/Y H:i').' ) '.'<br>ผู้ป่วย : (hn : '.$hn_row_ipt.')(an : '.$an_REQUEST.')(ชื่อ - สกุล : '.$pname_row_ipt.' '.$fname_row_ipt.' '.$lname_row_ipt.')'.' ( Page {PAGENO} of {nb} )');
                $mpdf2->WriteHTML('');
                //----------------------select ข้อมูล จากฐานข้อมูลเพื่อค้นหา ชื่อ - สกุล
...
$mpdf2->setHTMLHeader($html_header);
$mpdf2->WriteHTML($head);
$mpdf22->Output();
?>