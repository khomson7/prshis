        ...
        <div class="patient-info-container-mini d-none d-print-none alert alert-secondary fixed-top" role="alert" style="z-index: 601;">
            <div class="d-flex">
                ...
                <div class="p-1 flex">
                    ...
                    <label class="patient-info-hn-an">HN : <?=htmlspecialchars($row_ipt['hn']);?> | AN : <?=htmlspecialchars($row_ipt['an'])?> | </label>
                    ...
                    <label class="patient-info-ward">ตึก : <?=htmlspecialchars($row_ipt['ward_name'])?> | </label>
                    <label class="patient-info-bed">เตียง : <?=htmlspecialchars($row_ipt['bedno'])?> | </label>
                    <label class="patient-info-pttype-name">สิทธิ : (<?=htmlspecialchars($row_ipt['pttype'])?>) <?=htmlspecialchars($row_ipt['pttype_name'])?></label>
                    ...
                    ?>
                    <label class="patient-info-regdate text-primary">วันที่ Admit : <?=htmlspecialchars($newDate." ".$newTime);?></label>
                    <?php if($row_ipt['dchdate'] != ""){
                        ...
                        ?>
                        | <label class="patient-info-dchdate text-warning">วันที่ Discharge : <?=htmlspecialchars($dchdate." ".$dchtime);?> - Discharge Status: (<?=htmlspecialchars($row_ipt['dchstts_name'])?>)</label>
                    <?php } ?>
                    <?php
                        if($row_ipt['drugallergy'] != "" || $row_ipt['allergy_drug_history'] != "" || $row_ipt['er_drugallergy_history'] != ""){
                            if($row_ipt['drugallergy'] != ""){?>
                                | <label class="patient-info-drugallergy text-danger font-weight-bold">แพ้ยา : <?=htmlspecialchars($row_ipt['drugallergy']);?></label><?php
                            }
                            ...
                            | <label class="patient-info-drugallergy">ยังไม่มีข้อมูลการแพ้ยา</label><?php
                        }
                        if($row_ipt['latest_bw'] != null && $row_ipt['latest_bw'] != ""){?>
                            | <label class="patient-info-bw">น้ำหนักตัวล่าสุด : <?=htmlspecialchars((float)($row_ipt['latest_bw']));?> kg (<?=htmlspecialchars(date("d/m/Y H:i", strtotime($row_ipt['latest_bw_datetime'])))?>) </label><?php
                        }
                    ?>
                </div>
            </div>
