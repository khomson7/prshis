...
$canEdit = Session::checkPermission('OPNOTE', 'EDIT');
?>
...
<div class="container-fluid py-2">
    ...
    <div class="iam-header">
        <div>
            <h5><i class="fas fa-images mr-2"></i>บันทึกข้อมูล Opnote</h5>
            <small>AN: <?= htmlspecialchars($an) ?></small>
        </div>
        <div>
            <?php if ($canEdit && ReportQueryUtils::checkReadOnly($an)): ?>
            <a href="form-opnote.php?an=<?= urlencode($an) ?>&loginname=<?= urlencode($loginname) ?>"
               target="_blank"
                    ...
                    <?php else: ?>
                        ...
                        <tr id="row-<?= $row['id'] ?>"
                            ...
                            <td class="text-center">
                                <a href="form-opnote.php?an=<?= urlencode($an) ?>&id=<?= $row['id'] ?>&loginname=<?= urlencode($loginname) ?>"
                                   target="_blank"
                                   class="btn btn-xs btn-outline-primary" style="font-size:0.78rem; padding:2px 8px;">
                                    <i class="fas fa-edit"></i> เปิด/แก้ไข
                                </a>
                                <a href="../pdffile/opnote-pdf.php?an=<?= urlencode($an) ?>&id=<?= $row['id'] ?>&loginname=<?= urlencode($loginname) ?>"
                                   target="_blank"
