...
try {
    ...
    Session::insertSystemAccessLog(json_encode(array(
        'form' => 'ORS-COMPLICATION-MAIN',
        'an'   => $an,
    ), JSON_UNESCAPED_UNICODE));

    $stmt = $conn->prepare(
        "SELECT id, create_datetime, update_datetime, create_user, update_user,
                nurse_name, nurse_name2, nurse_name3
           FROM " . DbConstant::KPHIS_DBNAME . ".prs_or_complication
...
<div class="container-fluid py-2">
    ...
    <div class="nfm-header">
        <div>
            <h5><i class="fas fa-notes-medical mr-2"></i>ใบประเมินภาวะแทรกซ้อนหลังการระงับความรู้สึกใน 24-48 ชั่วโมง</h5>
            <small>AN: <?= htmlspecialchars($an) ?></small>
        </div>
        <div>
            <?php if ($canEdit && \ReportQueryUtils::checkReadOnly($an)): ?>
            <a href="or-complication.php?an=<?= urlencode($an) ?>&loginname=<?= urlencode($loginname) ?>"
               onclick="window.open(this.href, 'or_comp_window'); return false;"
                ...
                <table class="table table-bordered table-sm table-hover mb-0 nfm-table">
                    <thead>
                        <tr>
                            ...
                            <th>บันทึกเมื่อ</th>
                            <th>ผู้ตรวจสอบ (Intra-op)</th>
                            <th>ผู้ตรวจสอบ (PACU)</th>
                            <th>ผู้ตรวจสอบ (Post-op)</th>
                            <th>ผู้บันทึก</th>
                    ...
                    <?php else: ?>
                        ...
                        <tr id="row-<?= $row['id'] ?>"
                            ...
                            <td><?= htmlspecialchars($row['nurse_name2'] ?: '-') ?></td>
                            <td><?= htmlspecialchars($row['nurse_name3'] ?: '-') ?></td>
                            <td><?= htmlspecialchars($row['create_user'] ?: '-') ?></td>
                            <td class="text-center">
                                <a href="or-complication.php?an=<?= urlencode($an) ?>&id=<?= $row['id'] ?>&loginname=<?= urlencode($loginname) ?>"
                                   onclick="window.open(this.href, 'or_comp_window_<?= $row['id'] ?>'); return false;"
                                   class="btn btn-xs btn-outline-primary" style="font-size:0.78rem; padding:2px 8px;">
                                    <i class="fas fa-edit"></i> เปิด/แก้ไข
                                </a>
                                <a href="or-complication-pdf.php?an=<?= urlencode($an) ?>&id=<?= $row['id'] ?>&loginname=<?= urlencode($loginname) ?>"
                                   target="_blank"
...
function deleteRecord(id, label) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('or-complication-delete.php',
